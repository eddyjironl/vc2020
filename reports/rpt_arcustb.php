	...
	while($row = mysqli_fetch_assoc($lcgrp_g)){
		...
		if ($lctype_stado == "rango"){
			...
			$lcsql  = " SELECT arcash.cinvno, arcasm.ccashno,
						...
						WHERE arcasm.dtrndate >= '". $dtrndate_1 ."'  and arcasm.dtrndate <= '". $dtrndate_2 ."'  and
...
function get_balance_inicial(){


select sum(arinvc.nsalesamt + arinvc.ntaxamt - arinvc.ndesamt ) AS nsaldo 
from arinvc 
left outer join arcust on arcust.ccustno = arinvc.ccustno 
left outer join arresp on arresp.crespno = arcust.crespno 
left outer join artcas on artcas.cpaycode = arinvc.cpaycode 
where arinvc.lvoid = 0 and 
	  arcust.ccustno = '001' and 
      arinvc.dstar < '2020-08-01' ;
      
      
SELECT sum(arcash.namount) AS npayamt 
FROM arcasm  
join arcash on arcasm.ccashno = arcash.ccashno
WHERE arcasm.dtrndate  < '2020-08-01' and
		arcasm.ccustno = '001' and
		arcasm.cstatus  = 'OP'
