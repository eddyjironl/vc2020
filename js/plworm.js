function init(){
	...
	gckeydesc = "cdesc";
	gcbtkeyid = "btcworkno";
	document.getElementById("btcworkno").addEventListener("click",function(){
...
function cerrar_pantalla_principal(){
	document.getElementById("plworm").style.display="none";
}
function guardar(){
	var oform = document.getElementById("plworm");
	// validaciones de campos obligatorios.
	if(document.getElementById("cworkno").value ==""){
...
function borrar(){
	var xkeyid = document.getElementById("cworkno").value;
	if(xkeyid != ""){
		...
		oDatos.append("cworkno",xkeyid);
		oDatos.append("accion","DELETE");
		// obteniendo el menu
		oRequest.open("POST","../modelo/crud_plworm.php",false); 
...
function valid_key(){
	var lcxkeyvalue = document.getElementById("cworkno").value;
	if (lcxkeyvalue != ""){	
		update_window(lcxkeyvalue,"btcworkno");
	}	
}
function update_window(pckeyid){
	...
	if (odata != null){
		document.getElementById("cworkno").value = odata.cworkno;
