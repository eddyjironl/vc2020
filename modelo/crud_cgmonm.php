...
if($lcaccion=="NEW"){
	...
	if (isset($_POST["cmonid"])){
		...
		$lcsimbolo = mysqli_real_escape_string($oConn, $_POST["csimbolo"]);
		$lcmetodo = mysqli_real_escape_string($oConn, $_POST["cmetodo"]);
		// verificando que el codigo exista o no 
		$lcsql   = " select cmonid from cgmonm where cmonid ='$lcmonid' ";
		...
		if ($lnCount == 0){
			...
			$lcsqlcmd = " insert into cgmonm (cmonid,cdesc,mnotas, csimbolo, cmetodo)	values('$lcmonid','$lcdesc','$lmnotas','$lcsimbolo','$lcmetodo')";
		}else{
			// el codigo existe lo que hace es actualizarlo.	
			$lcsqlcmd = " update cgmonm set cdesc = '$lcdesc',mnotas = '$lmnotas', csimbolo = '$lcsimbolo', cmetodo = '$lcmetodo' where cmonid = '$lcmonid' ";
		}
		// ------------------------------------------------------------------------------------------------
	...
	header("location:../view/cgmonm.php");		
}  		//if($lcaccion=="NEW")
// ------------------------------------------------------------------------------------------------
