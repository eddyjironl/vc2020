...
if($lcaccion=="NEW"){
	...
	if (isset($_POST["cctaid"])){
        ...
        $llogoBC   = isset($_POST["llogoBC"]) ? 1:0;  
        $llogoBG   = isset($_POST["llogoBG"]) ? 1:0;  
        $llogoER   = isset($_POST["llogoER"]) ? 1:0;  
        $lnrentax  = mysqli_real_escape_string($oConn,$_POST["nrentax"]);
        $ngrupid   = mysqli_real_escape_string($oConn,$_POST["ngrupid"]);
        $lnConfRChk= isset($_POST["lnConfRChk"]) ? 1:0;
        ...
        /* Sangria de grupos */
        $lnmic1desc = mysqli_real_escape_string($oConn,$_POST["nmic1desc"]);
        $lnmic2desc = mysqli_real_escape_string($oConn,$_POST["nmic2desc"]);
        $lnmic3desc = mysqli_real_escape_string($oConn,$_POST["nmic3desc"]);
        $lnmic4desc = mysqli_real_escape_string($oConn,$_POST["nmic4desc"]);
        $lnmic5desc = mysqli_real_escape_string($oConn,$_POST["nmic5desc"]);
		...
		$lcsql   = " select cctaid from cgsetup ";
		...
		if ($lnCount == 0){
			...
			$lcsqlcmd = " insert into cgsetup (cperid,cmonid,nrentax,ngrupid,lnConfRChk
							...
							values('$lcperid','$lcmonid',$lnrentax,$ngrupid,$lnconfrchk,
			...
			$lcsqlcmd = " update cgsetup set cperid='$lcperid',cmonid = '$lcmonid',nrentax = $lnrentax,ngrupid = $ngrupid, lnconfrchk = $lnconfrchk,
